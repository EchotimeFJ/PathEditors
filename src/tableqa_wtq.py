...
def reasoning_llm(question, instantiate_path):
    """
    调用大语言模型(LLM)对表格问答进行推理，获取最终答案。

    Args:
        question (str): 用户提出的问题
        instantiate_path (str): 实例化后的表格数据，包含问题相关的表格内容

    Returns:
        list: LLM推理得出的答案列表，非空且格式规范
    """
    # 读取表格问答推理专用的提示模板
    prompt = open(
    ...
    # 构建完整提示，将问题和实例化表格数据添加到模板中
    append = f"""\nQuestion: {question}\n<Table>\n{instantiate_path}\n</Table>\n<Thought>\n"""
    prompt += append

    # 最多重试MAX_LLM_RETRY_TIME次调用LLM
    for _ in range(MAX_LLM_RETRY_TIME):
        try:
            # 调用LLM，传入提示和参数
            response = run_llm(prompt, options.temperature, options.max_token, options.openai_api_keys,
                               options.LLM_type)

            # 从响应中提取答案部分（Answer:之后的内容）
            ...
            break  # 成功获取有效答案，退出重试循环

        except Exception as e:
            # 处理异常情况，打印错误信息
            print("****************************************************")
            print(answer)  # 打印当前答案（可能格式有误）
            print("****************************************************")
            answer = []  # 重置答案为空列表
            continue  # 继续重试

    return answer  # 返回最终的答案列表
